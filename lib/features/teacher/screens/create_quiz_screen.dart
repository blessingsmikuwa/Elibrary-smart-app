...
class _Question {
  ...
  Map<String, dynamic> toJson() =>
      {'id': id, 'text': text, 'options': options, 'answer': answer};
}
...
_Question _blankQ() =>
    _Question(id: '${DateTime.now().microsecondsSinceEpoch}-${Object().hashCode}');
...
class _CreateQuizScreenState extends State<CreateQuizScreen> {
  ...
  Future<void> _loadQuizzes() async {
    ...
    try {
      final res = await http.get(
          Uri.parse('$kApiBase/quizzes/mine'),
          headers: await _headers());
      if (res.statusCode < 300)
        setState(() => _savedQuizzes =
  ...
  Future<void> _handleSave() async {
    if (_quiz.title.trim().isEmpty) {
      _showToast('Please enter a quiz title.', type: 'error');
      return;
    }
    if (_quiz.questions.any((q) => q.text.trim().isEmpty)) {
      _showToast('All questions must have text.', type: 'error');
      return;
    }
    if (_quiz.questions
        .any((q) => q.options.any((o) => o.trim().isEmpty))) {
      _showToast('Please fill in all answer options.', type: 'error');
      return;
    }
    ...
    try {
      final body = jsonEncode({
        ...
        'visibility': _quiz.visibility,
        'schoolId': _quiz.schoolId,
        'mode': _mode,
        'status': 'published',
        'questions': _quiz.questions.map((q) => q.toJson()).toList(),
      });
      final res = await http.post(
          Uri.parse('$kApiBase/quizzes'),
          headers: await _headers(),
          body: body);
      if (res.statusCode >= 300) {
        final d = jsonDecode(res.body);
        throw Exception(d['message'] ?? 'Failed to save quiz');
      }
      final saved = jsonDecode(res.body) as Map<String, dynamic>;
      setState(() {
        _savedQuizzes = [saved, ..._savedQuizzes];
        _quiz = _blankQuiz();
        _view = 'saved';
      });
      if (_mode == 'offline') {
      ...
      _showToast(e.toString(), type: 'error');
    } finally {
  ...
  Future<void> _exportPdf(dynamic quizId) async {
    try {
      ...
      if (res.statusCode >= 300) {
        final d = jsonDecode(res.body);
        throw Exception(d['message'] ?? 'PDF export failed');
      }
      ...
      if (await canLaunchUrl(uri)) {
        ...
        _showToast('PDF ready! Opening download…');
      } else {
  ...
  Future<void> _deleteQuiz(dynamic id) async {
    try {
      final res = await http.delete(
          Uri.parse('$kApiBase/quizzes/$id'),
          headers: await _headers());
      if (res.statusCode >= 300) throw Exception('Failed to delete quiz');
      setState(() =>
          _savedQuizzes = _savedQuizzes.where((q) => q['id'] != id).toList());
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        ...
        if (_toast != null) _buildToast(t),
      ]),
    );
  }

  Widget _buildToast(AppThemeData t) {
      ...
      child: Material(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: isErr ? t.redBg : t.greenBg,
            border: Border.all(color: isErr ? t.danger : t.primary),
          ...
          child: Row(children: [
            ...
            Expanded(
              child: Text(
                _toast!['msg'],
                style: TextStyle(
                    color: isErr ? t.redText : t.greenAccent,
    ...
    return SingleChildScrollView(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_view == 'create') _buildCreateView(t),
          if (_view == 'saved')  _buildSavedView(t),
  ...
  Widget _buildHeader(AppThemeData t) {
    final tabs = [
      {'key': 'create', 'label': '✏️ Create'},
      {
        'key': 'saved',
        'label':
            '📋 My Quizzes${_savedQuizzes.isNotEmpty ? " (${_savedQuizzes.length})" : ""}'
      },
    ];
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('✏️ Quiz Management',
            style: TextStyle(
                color: t.text,
                fontWeight: FontWeight.bold,
                fontSize: 20)),
        const SizedBox(height: 4),
        Text('Build quizzes and track student progress.',
            style: TextStyle(color: t.muted, fontSize: 13)),
        const SizedBox(height: 16),
        Row(
          children: tabs.map((tab) {
            final active = _view == tab['key'];
            return Expanded(
              child: Padding(
                ...
                child: GestureDetector(
                  onTap: () {
                    setState(() => _view = tab['key']!);
                    if (tab['key'] == 'saved') _loadQuizzes();
                  },
                  child: Container(
                    padding:
  ...
  Widget _buildCreateView(AppThemeData t) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Expanded(
            child: _tabBtn(t, '🌐 Online Quiz', _mode == 'online',
                () => setState(() => _mode = 'online'))),
        const SizedBox(width: 8),
        Expanded(
            child: _tabBtn(t, '📄 Offline / Printable', _mode == 'offline',
      ...
      Container(
        ...
        padding:
            const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        decoration: BoxDecoration(
          color: _mode == 'online' ? t.blueBg : t.surface2,
          border: Border.all(
              color: _mode == 'online' ? t.blueText : t.border),
          borderRadius: BorderRadius.circular(8),
        ),
        child: Text(
          _mode == 'online'
          ...
          style: TextStyle(
              color: _mode == 'online' ? t.blueText : t.muted,
              fontSize: 12),
        ),
      ),
      const SizedBox(height: 16),
      _buildQuizDetails(t),
      const SizedBox(height: 16),
      _buildQuestionsSection(t),
      const SizedBox(height: 16),
      _buildSaveBar(t),
    ]);
  }

  Widget _tabBtn(AppThemeData t, String label, bool active,
      VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border:
              Border.all(color: active ? t.primary : t.cardBorder),
          borderRadius: BorderRadius.circular(8),
        ),
        child: Text(label,
            textAlign: TextAlign.center,
  ...
  Widget _buildQuizDetails(AppThemeData t) {
    return _card(
      t,
      title: '📝 Quiz Details',
      children: [
        _lbl(t, 'Quiz Title *'),
        _textField(t,
            hint: 'e.g. Cell Biology Quiz — Week 3',
            value: _quiz.title,
        ...
        _dropdown(t,
            ...
            items: _subjects,
            onChanged: (v) =>
                setState(() => _quiz.subject = v ?? _quiz.subject)),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(
              child: Column(
                ...
                _dropdown(t,
                    ...
                    items: _forms,
                    onChanged: (v) =>
                        setState(() => _quiz.form = v ?? _quiz.form)),
              ])),
          ...
          Expanded(
              child: Column(
                ...
                _lbl(t, 'Duration'),
                _dropdown(t,
                    value: _quiz.duration,
                    items: _durations,
                    onChanged: (v) => setState(
                        () => _quiz.duration = v ?? _quiz.duration)),
              ])),
        ]),
        const SizedBox(height: 12),
        _lbl(t, 'Visibility'),
        _dropdown(
          ...
          display: (v) => v == 'PUBLIC'
              ...
              : 'Private (School Only)',
          onChanged: (v) =>
              setState(() => _quiz.visibility = v ?? _quiz.visibility),
        ),
        if (_quiz.visibility == 'PRIVATE') ...[
          ...
          _dropdown(
            t,
            value: _quiz.schoolId.isEmpty && _schools.isNotEmpty
                ? ''
                : _quiz.schoolId,
            items: ['', ..._schools.map((s) => s['id'].toString())],
            display: (v) {
              if (v.isEmpty) return 'Select school';
              final s = _schools.firstWhere(
                  (s) => s['id'].toString() == v,
                  orElse: () => {});
              return s['name']?.toString() ?? v;
            },
            onChanged: (v) =>
                setState(() => _quiz.schoolId = v ?? ''),
          ),
        ],
        const SizedBox(height: 12),
        _lbl(t, 'Description (optional)'),
        _textField(t,
            hint: 'Brief instructions or topic overview...',
            value: _quiz.description,
            maxLines: 2,
            onChanged: (v) =>
                setState(() => _quiz.description = v)),
      ],
    );
  }

  Widget _buildQuestionsSection(AppThemeData t) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text('❓ Questions (${_quiz.questions.length})',
            style: TextStyle(
                ...
                fontWeight: FontWeight.bold,
                fontSize: 14)),
        _smallBtn(t, '+ Add Question', onTap: _addQuestion),
      ]),
      const SizedBox(height: 12),
      ...List.generate(
          ...
          (i) => _QuizQuestionCard(
                ...
                onChange: (updated) =>
                    setState(() => _quiz.questions[i] = updated),
                onRemove: () =>
                    setState(() => _quiz.questions.removeAt(i)),
              )),
  ...
  void _addQuestion() =>
      setState(() => _quiz.questions.add(_blankQ()));

  Widget _buildSaveBar(AppThemeData t) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: t.surface,
      ...
      child: Column(children: [
        Row(children: [
          Text('${_quiz.questions.length}',
              style: TextStyle(
                  color: t.text,
                  fontWeight: FontWeight.bold,
                  fontSize: 13)),
          Text(
              ' question${_quiz.questions.length != 1 ? "s" : ""} · ',
          ...
          Text(
            ...
            style: TextStyle(
                color:
                    _mode == 'online' ? t.blueText : t.amberText,
                fontWeight: FontWeight.w600,
                fontSize: 13),
          ),
        ]),
  ...
  Widget _buildSavedView(AppThemeData t) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        RichText(
          ...
          TextSpan(
              ...
              style: TextStyle(
                  ...
                  fontSize: 17)),
          TextSpan(
              text: '(${_savedQuizzes.length} total)',
              style: TextStyle(color: t.subtle, fontSize: 13)),
        ])),
        ElevatedButton(
      ...
      if (_loadingList)
        ...
        ...(_savedQuizzes
            .map((q) => _SavedQuizCard(t: t, quiz: q, onDelete: _deleteQuiz))),
    ]);
  }

  Widget _card(AppThemeData t,
  ...
  Widget _textField(AppThemeData t,
  ...
  Widget _dropdown(AppThemeData t,
          {required String value,
      ...
      DropdownButtonFormField<String>(
        ...
        items: items
            .map((i) => DropdownMenuItem(
                value: i,
                child: Text(display != null ? display(i) : i)))
            .toList(),
      );

  Widget _smallBtn(AppThemeData t, String label,
    ...
    return Container(
      ...
      child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            Text('Click the circle to mark correct answer',
...
class _SavedQuizCard extends StatefulWidget {
  ...
  const _SavedQuizCard(
      {required this.t, required this.quiz, required this.onDelete});

  @override
  State<_SavedQuizCard> createState() => _SavedQuizCardState();
}
...
class _SavedQuizCardState extends State<_SavedQuizCard> {
              ...
              children: [
                Row(children: [
                  _chip(t,
                      ...
                      isOnline ? t.blueText : t.amberText,
                      isOnline ? t.blueBg   : t.amberBg),
                  const SizedBox(width: 6),
                  _chip(t, q['subject']?.toString() ?? '',
                      t.greenAccent, t.greenBg),
                  const SizedBox(width: 6),
                  _chip(t, q['form']?.toString() ?? '',
                      t.muted, t.surface2, border: t.border),
                  const Spacer(),
                  GestureDetector(
                ...
                const SizedBox(height: 10),
                Row(children: [
                  Text('❓ ${qs.length} questions',
                      style: TextStyle(color: t.subtle, fontSize: 12)),
                  const SizedBox(width: 12),
                  Text('⏱ ${q['duration'] ?? ''}',
                      style: TextStyle(color: t.subtle, fontSize: 12)),
                  const SizedBox(width: 12),
                  Text('📅 ${_fmtDate(q['createdAt'])}',
                      style: TextStyle(color: t.subtle, fontSize: 12)),
                ]),
                const SizedBox(height: 12),
                Row(children: [
                          ...
                          Wrap(
                            ...
                            children: List.generate(opts.length, (oi) {
                              ...
                              return Container(
                                ...
                                child: Text(
                                  '${String.fromCharCode(65 + oi)}. ${opts[oi]}${isAns ? " ✓" : ""}',
                                  style: TextStyle(
                                      color: isAns
                                          ? t.greenAccent
                                          : t.subtle,
  ...
  Widget _chip(AppThemeData t, String label, Color color, Color bg,
          {Color? border}) =>
      Container(
        padding:
